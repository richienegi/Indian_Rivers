...
/*check*/
    Context c;
    ArrayList<QuizClass> data;
    ArrayList<String> ans;

    public AnswerList(Context c, ArrayList<QuizClass> data, ArrayList<String> ans)
    {
        this.c=c;
        this.data=data;
        this.ans=ans;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        t1.setText(position+1+") "+data.get(position).question);
        if(!ans.get(position).trim().equalsIgnoreCase(data.get(position).answer.trim()))
        {
            img.setImageResource(R.drawable.wrong);
        }
        else {
            img.setImageResource(R.drawable.correct);
            //t3.setBackgroundColor(Color.parseColor("#008000"));
        }
        t2.setText(data.get(position).answer);
